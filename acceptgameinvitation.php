        ...
        if (mysqli_num_rows($result5) == 1) {
            ...
            if ($thisgamename != "") {
                ...
                if (mysqli_num_rows($result7) == 0) {
                    echo 'this case should never be there. must be error in query';
                } else {
                ...
                if (!$thisusernotpartofsamegame) {
                    ...
                    if (mysqli_affected_rows($dbc) != 0) {
                        ...
                        if (mysqli_num_rows($result3) > 0) {
                            ...
                            if ($gametype == "slots" || $gametype == 'omaha')
                                $playerstatus = 1;
                            ...
                            echo '<result><status>1</status><session_id>' . $sessionid . '</session_id><game_type>' . $thisgamename . '</game_type></result>';
                            ...
                            $query6 = "select user_id from games_players where session_id = $sessionid and user_id not in (-5, $userid)";
                            $result6 = mysqli_query($dbc, $query6);

                            // set this message as latest massage of player
                            $message2 = 'Joined the Game';

                            $latesthandlerid = setlatestmessageid($sessionid, $userid, $message2, 1, $dbc);

                            $message3 = 'Started Playing ' . ucfirst(fetchgametype($sessionid, $dbc));
                            insertIntoFeed($userid, $message3, $dbc);

                            giveRewardPoint($userid, $thisgamename, $gotsessionid, $dbc);
