    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        if ((time() - strtotime($roomrentdeductiontime)) > 60 * 60 * 24) {
            ...
            if (mysqli_num_rows($result3) > 0) {
                ...
                if ($olderchips < $chipstodeduct) {
                    if ($olderchips > 0) {
                        $message = "The Hotel has charged you '. $olderchips .' chips for your $roomname.";
                        $querym = "insert into messages(message, message_type, user_id, handler_id) values ('$message', 3, $userid, 0)";
                        mysqli_query($dbc, $querym);
                    }
                } else {
                    $message = "The Hotel has charged you '. $chipstodeduct .' chips for your $roomname.";
                    $querym = "insert into messages(message, message_type, user_id, handler_id) values ('$message', 3, $userid, 0)";
                    mysqli_query($dbc, $querym);
                }
            }
