...
// no invitation is sent to this user
        ...
        if (is_null($gametype)) {
            ...
            if (mysqli_num_rows($result4) == 0) {
                ...
                $msg = "Invites ";
                $invitedusersexploded = explode(',', $useridslist);

                $names = array();
                for ($i = 0; $i < count($invitedusersexploded); $i++) {
                    array_push($names, fetchname($invitedusersexploded[$i], $dbc));
                }
                $msg .= join(',', $names);
                $msg .= " to play " . fetchgametype($sessionid, $dbc);


                insertIntoFeed($userid, $msg, $dbc);
                ...
                while ($row4 = mysqli_fetch_array($result4)) {
                    ...
                    if (!is_null($inviteddevicetoken)) {
                        ...
                        // form a message now 
                        $messageformed = $nameofrequestsender . ' wants you to play ' . $gametype;
...
$handleridlatestmessage = 0;
if ($invited) {

    if (count($pusharray1) == 1)
        $latestmessage = 'Invited Friend';
    else
        $latestmessage = 'Invited Friends';
...
if (count($pusharray1) > 0) {
    ...
    $message = $nameofrequestsender . ' wants you to play ' . $gametype;
