...
else
{
    ...
    else
    {
        ...
        $olderBetAmount = 0;
        $queryoba= "select amount from blackjack_bets where user_id = $userid and session_id = $sessionid";
        $resultoba = mysqli_query($dbc, $queryoba);
        
        if(mysqli_num_rows($resultoba)> 0)
        {
            $rowoba = mysqli_fetch_row($resultoba);
            $olderBetAmount = $rowoba[0];
        }
        ...
        if (mysqli_query($dbc, $query1))
        {
            ...
            $chipstodeduct = convertChips($amount) - convertChips($olderBetAmount);
            increasedecreasechips($userid, $chipstodeduct, 0, $dbc);
            ...
            if (mysqli_num_rows($result2) == 0)
            {
                ...
                if (mysqli_num_rows($resultm1) > 0)
                {
                    ...
                    $query3  = "select user_id from blackjack_bets where session_id = $sessionid and user_id !=0";
                    ...
                    sendpushtoplayers($pushids, $message, $dbc);
                }
            }
        }
        else
