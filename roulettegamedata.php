        ...
        if (mysqli_num_rows($result12) != 0) {
            ...
            if (mysqli_num_rows($result10) > 0) {
                ...
                roulette_bets.cases !='' ";
                    ...
                    $newdatewnt = date('Y-m-d  H:i:s', strtotime(date('Y-m-d  H:i:s') . '+5 seconds'));
                    $querywnt = "update roulette_game_data set winning_number_datetime = '$newdatewnt' where session_id = $sessionid";
                    mysqli_query($dbc, $querywnt);
                        ...
                        while ($row = mysqli_fetch_array($result)) {
                            ...
                            for ($i = 0; $i < count($casessplitted); $i++) {
                                ...
                                if ($wonornot) {
                                    ...
                                    $playerwinsorloses += $amounttoincrease - convertChips($amountsplitted[$i]);
                                } else {
...
//                                    increasedecreasechips($userid, $amounttodecrease, 1, $dbc);
                            ...
                            if ($playerwinsorloses == 0) {
                                ...
                                $queryconfetti = "update roulette_bets set confetti_status = '$newdate', confetti_value=1 where session_id = $sessionid and user_id= $userid";
                                mysqli_query($dbc, $queryconfetti);

                                publishDailyWinnings($userid, $playerwinsorloses, 'chips', $dbc);
                            } else {
                                $pushids[] = array($userid, $playerwinsorloses);
                                $message = '- ' . convertBackChips(-$playerwinsorloses);
                                addWinningsLosings($userid, $sessionid, $playerwinsorloses, true, $dbc);
                                ...
                                $newdate = date('Y-m-d  H:i:s', strtotime(date('Y-m-d  H:i:s') . '+5 seconds'));
                                $queryconfetti = "update roulette_bets set confetti_status = '$newdate', confetti_value=2 where session_id = $sessionid and user_id= $userid";
                                mysqli_query($dbc, $queryconfetti);
                    ...
                    $activeusers = array(-5);
                    for ($in = 0; $in < count($pushids); $in++) {
                        array_push($activeusers, $pushids[$in][0]);
                    }

                    $activeuserstext = join(',', $activeusers);

                    $queryn1 = "select user_id from roulette_bets where session_id = $sessionid and user_id not in ($activeuserstext)";
                    $result1 = mysqli_query($dbc, $queryn1);
                    if (mysqli_num_rows($result1) > 0) {
                        while ($row1 = mysqli_fetch_array($result1)) {
                            $pushids[] = array($row1[0], 0);
                        }
                    }


                    // get a color for returning 
                    $black = array(0);
                    $green = array(37);
                    ...
                    $color = "";
                    if (in_array($winningnumber, $black)) {
                        $color = "Black";
                    } elseif (in_array($winningnumber, $green)) {
                        $color = "Green";
                    } elseif (in_array($winningnumber, $red)) {
                        $color = "Red";
                    } else {
                        $color = "Black";
                    }
                    ...
                    for ($i = 0; $i < count($pushids); $i++) {
                        if ($pushids[$i][1] > 0) {
                            $body['aps'] = array(
                                'alert' => "Roulette Lucky Number is $color: $winningnumber \n You won " . number_format($pushids[$i][1]) . " chips.",
                                'sound' => '10'
                            );
                        } elseif ($pushids[$i][1] < 0) {
                            $body['aps'] = array(
                                'alert' => "Roulette Lucky Number is $color: $winningnumber \n You lose " . number_format(-$pushids[$i][1]) . " chips.",
                                'sound' => '10'
                            );
                        } else {
                            $body['aps'] = array(
        ...
        if (mysqli_num_rows($result2) != 0) {
            ...
            $query = "select user_details.user_id,games_players.status,invitations.status, name, gold, chips, icon_name, amount, roulette_bets.confetti_status, roulette_bets.confetti_value
                from 
            ...
            $queryfetchwinningnumberdate = "select winning_number_datetime from roulette_game_data where session_id = $sessionid";
            $resultwnd = mysqli_query($dbc, $queryfetchwinningnumberdate);

            if (mysqli_num_rows($resultwnd) > 0) {
                $rowfetchwn = mysqli_fetch_row($resultwnd);

                if (strtotime($rowfetchwn[0]) > time()) {
                    echo '<winningNumber>';
                    $querywn = "select winning_number from roulette_game_data where session_id = $sessionid";
                    $resultwn = mysqli_query($dbc, $querywn);
                    if (mysqli_num_rows($resultwn) > 0) {
                        $rowwn = mysqli_fetch_row($resultwn);
                        echo $rowwn[0];
                    }
