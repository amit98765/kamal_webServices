...
else
{
    ...
    if (!$done)
    {
        echo '<status>'. $giftprice .'</status>';
    }
    else
    {
        $handlerid = NULL;

        $feedmsg   = "Gifted " . $giftname;
        $feedarray = array();
        ...
        for ($i = 0; $i < count($useridsexploded); $i++)
        {
            if ($giftname == "Jeffrey")
            {
                $querymy1 = "update user_cash set jeffrey = jeffrey - 1 where user_id = $userid";
                mysqli_query($dbc, $querymy1);
            }

            array_push($feedarray, fetchname($useridsexploded[$i], $dbc));
            ...
            $message = $name . ' sent you a gift while playing ' . $gametype;


            $feedmsg1 = "Received " . $giftname . ' in ' . $gametype;
            insertIntoFeed($useridsexploded[$i], $feedmsg1, $dbc);
