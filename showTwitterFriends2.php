...
if (is_null($_GET['user_id']) || is_null($_GET['friends_ids']) || is_null($_GET['twitter_id']))
{
...
else
{
    ...
    for ($i                    = 0; $i < count($twitteridsexploded); $i++)
    {
        ...
        $query = "select user_details.user_id, user_details.email_id, user_details.name, icon_name 
            ...
            having
            user_details.user_id in (select user_id from user_details where twitter_id like '%" . $twitteridsexploded[$i] . "%' and user_id != $userid)";
